...
export default class ProductsAdmin extends React.Component {
    ...
    updateProduct = (id, img, title, model, price, idCategory, description, count, isPopular) => {
        for (let i = 0; i < this.state.records.length; i++) {
            if (this.state.records[i].id === id) {
                const mas = [...this.state.records]
                mas.splice(i, 1, { id, img, title, model, price, idCategory, description, count, isPopular })
                this.setState({ records: mas })
            }
        }
    }
    AddProduct(id_, img, title, model, price, idCategory, description, count, isPopular) {
        this.props.getProducts()
        setTimeout(() => {
            const id = this.props.idProductNow
            const mas = [...this.state.records]
            mas.splice(this.state.records.length, 1, { id, img, title, model, price, idCategory, description, count, isPopular })
            this.setState({ records: mas })
        }, 1500)
    }
}
